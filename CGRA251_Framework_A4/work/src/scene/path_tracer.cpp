...
vec3 CorePathTracer::sampleRay(const Ray &ray, int) {
	...
	// YOUR CODE GOES HERE
	// ...
	RayIntersection intersection = m_scene->intersect(ray);

	if(intersection.m_valid)
	{
		vec3& p = intersection.m_position;
		Material& m = *intersection.m_material;
		...
		{
			ambience += light->ambience() * 0.05f;
			if (!light->occluded(m_scene,p))
			{
				diffuse += dot(light->incidentDirection(p), intersection.m_normal) * light->irradiance(p) * m.diffuse();
				vec3 r = reflect(light->incidentDirection(p), intersection.m_normal);
				specular += light->irradiance(p) * m.specular() * pow(glm::max(0.0f, dot(normalize(r), normalize(-ray.direction))), m.shininess());
			}
		}
	...
	if(intersection.m_valid)
	{
		...
		for(const auto& light : m_scene->lights())
		{
			...
			if (!light->occluded(m_scene,p))
			{
				diffuse += dot(light->incidentDirection(p), intersection.m_normal) * light->irradiance(p) * m.diffuse();
				vec3 r = reflect(light->incidentDirection(p), intersection.m_normal);
				specular += light->irradiance(p) * m.specular() * pow(glm::max(0.0f, dot(normalize(r), normalize(-ray.direction))), 1 - (1/m.shininess()));
			}
		}
		return ambience + diffuse + specular;
	}
