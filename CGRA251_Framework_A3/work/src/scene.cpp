...
void Scene::loadCore()
{
	...
	for(int i = 0; i <= 100; i++)
	{
		...
		m_boids.push_back(Boid(linearRand(vec3(-50, -50, -50), vec3(50)), sphericalRand(40.0),1));
	}
...
void Scene::loadCompletion()
{
	...
	for (int i = 0; i <= 50; i++)
	{
		m_pradoter.push_back(Boid(linearRand(vec3(-25, -25, -25), vec3(75)), sphericalRand(40.0), 2));
	}
...
void Scene::draw(const mat4& proj, const mat4& view )
{
	...
	if (m_show_skymap)
	{
		...
		glUniform1i(glGetUniformLocation(m_skymap_shader, "uSkyMap"), 0);
	...
	for (const Boid& b : m_boids)
	{
		...
		// draw
		//
		// for(Boid obj : m_boids)
		// {
		// 	if (obj.getPradator() == 2) {
		// 		std::cout << obj.getPradator() << std::endl;
		// 		m_boid_mesh.draw();
		// 	}
		// 	else if(obj.getPradator()== 1)
		// 	{
	...
	for (const Boid& b : m_pradoter)
	{
		// get the boid direction (default to z if no velocity)
		vec3 dir = normalize(b.velocity());
		if (dir.x != dir.x) dir = vec3(0, 00, 1);
...
void Scene::renderGUI()
{
	...
	// YOUR CODE GOES HERE
	// ...
	ImGui::SliderFloat("neighbourhood radius", &this ->m_neighbourhood_radius, 0, 100);
	ImGui::SliderFloat("avoidance radius", &this->m_avoidance_radius, 0, 100);
	ImGui::SliderFloat("cohesion", &this -> m_cohesion_weight, 0, 10);
	ImGui::SliderFloat("alignment", &this -> m_alignment_weight, 0, 10);
	ImGui::SliderFloat("avoidance", &this -> m_avoidance_weight , 0, 10);

	ImGui::SliderFloat("Predator neighbourhood radius", &this->p_m_neighbourhood_radius, 0, 100);
	ImGui::SliderFloat("Predator avoidance radius", &this->p_m_avoidance_radius, 0, 100);
	ImGui::SliderFloat("Predator cohesion", &this->p_m_cohesion_weight, 0, 10);
	ImGui::SliderFloat("Predator alignment", &this->p_m_alignment_weight, 0, 10);
	ImGui::SliderFloat("Predator avoidance", &this->p_m_avoidance_weight, 0, 10);
}
